...
@app.route("/")
def index():
    homepage = "<h1>黃靜梅Python網頁</h1>"
    homepage += "<a href=/mis>MIS</a><br>"
    homepage += "<a href=/today>顯示日期時間</a><br>"
    homepage += "<a href=/welcome?nick=meim>傳送使用者暱稱</a><br>"
    homepage += "<a href=/about>靜梅簡介網頁</a><br>"
    homepage += "<br><a href=/search>讀取Firestore資料</a><br>"
    homepage += "<br><a href=/movie>讀取開眼電影即將上映影片，寫入Firestore</a><br>"
    homepage += "<br><a href=/searchh>電影查詢</a><br>"
...
def search():
    if request.method == "POST":
        ...
        result = "請輸入您要查詢的關鍵字：" + cond

    if request.method == "POST":
        aond = request.form["teacher"]
        ...
        for doc in docs:
            dict = doc.to_dict()
            if cond in dict["Course"] and aond in dict["Leacture"]:
...
@app.route("/searchh", methods=["POST","GET"])
def searchh():
    if request.method == "POST":
